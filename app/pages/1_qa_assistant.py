                ...
                for idx, source in enumerate(message["sources"]):
                    ...
                    with st.container():
                        ...
                        if col1.button(f"📋 Copy Citation", key=f"copy_{i}_{idx}"):
                            ...
                            components.html(f"<script>navigator.clipboard.writeText('{citation_text}');</script>", height=0)
                                ...
                                with st.expander("📄 Document Viewer", expanded=True):
                                    try:
                                        ...
                                        try:
                                            ...
                                            st.image(pil_image, caption=f"Page {page_num} of {doc_name}")
                                            with open(file_path, "rb") as f:
                                                st.download_button(label="📥 Download Full PDF", data=f, file_name=doc_name, mime="application/pdf", key=f"dl_{i}_{idx}")
                                        except Exception as page_e:
                                            st.error(f"Could not render page {page_num}: {page_e}")
                                        finally: pdf.close()
                                    except Exception as e: st.error(f"Could not load PDF: {e}")
...
def main():
    st.set_page_config(page_title="Q&A Assistant - Hemas PharmaComply AI", page_icon="💬", layout="wide")
    render_sidebar()
    if 'history_manager' not in st.session_state: st.session_state.history_manager = HistoryManager()
    if 'current_session_id' not in st.session_state: st.session_state.current_session_id = st.session_state.history_manager.create_new_session()
    if 'chatbot' not in st.session_state:
        config = load_config()
        st.session_state.chatbot = HemasPharmaComplyAI(config)
    chatbot = st.session_state.chatbot
    st.title("💬 Q&A Assistant")
    st.markdown("Ask deep questions across your entire NMRA regulation library.")
    st.divider()
